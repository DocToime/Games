...
let currentSpeed = displaySpeeds["Level 3"]; // Set the default speed to 500 milliseconds
...
function generateWordData(level) {
    let wordList;

    switch (level) {
        case "2":
            // For level 2, use words that are either 2 or 3 letters long
            wordList = two_threeLetterWords.filter(word => word.length === 2);
            break;
        case "3":
            // For level 3, use words that are 4 letters long
            wordList = two_threeLetterWords.filter(word => word.length === 3);
            break;        default:
            // For any other level, use any word length
            wordList = two_threeLetterWords;
    }

    let rounds = [];
    for (let i = 0; i < 10; i++) {
        let existingOptions = new Set();
        const word = selectRandomWord(wordList, existingOptions);
...
function generateChallengingOptions(correctWord, wordList) {
    ...
    while (options.size < 4) {
        ...
        if (similarWords && similarWords.length) {
            ...
            while (options.size < 4) {
                let randomWord = selectRandomWord(wordList, options);
...
function selectRandomWord(wordList, existingOptions) {
...
function startLevel(difficulty, level) {
    ...
    if (currentMode !== 'dynamic') {
        currentSpeed = displaySpeeds["Level 3"]; // Reset to default speed
    }

    // Differentiate data generation based on level
    if (difficulty === "Characters") {
        switch (level) {
            case "1":
                gameData[currentDifficulty][currentLevel] = generateLetterData(1, 'English');
                break;
            case "Hebrew":
                gameData[currentDifficulty][currentLevel] = generateLetterData(1, 'Hebrew');
                break;
            case "Cyrillic":
                gameData[currentDifficulty][currentLevel] = generateLetterData(1, 'Cyrillic');
                break;
            default:
                console.error('Invalid level for Characters difficulty');
                return;
        }
    } else {
        // Use generateWordData for other difficulties and levels
        gameData[currentDifficulty][currentLevel] = generateWordData(level, currentMode);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    speedSelector.value = "Level 3"; // Set default speed to Level 3
