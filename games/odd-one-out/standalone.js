...
function createWordButtons(words) {
    // Simulating wordOptionsContainer
    const wordOptionsContainer = { innerHTML: '', appendChild: (button) => console.log('Button added:', button) };
    ...
    words.forEach((word) => {
        const button = {
            innerText: word,
            style: { fontFamily: 'Arial', backgroundColor: 'blue', color: 'white', fontSize: '20px' },
            addEventListener: (event, callback) => console.log(`Event listener added for ${word}`)
        };
        wordOptionsContainer.appendChild(button);
    });
}

// Test with Beginner level 1
let words = gameData['Beginner']['1'][0].words;
